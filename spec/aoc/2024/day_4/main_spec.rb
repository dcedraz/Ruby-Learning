...
describe 'DayFour' do
  context 'Part I' do
    ...
    it 'checks if the direction is horizontal' do
      expect(day_four.horizontal_direction?(0, 0, 0, 1)).to be_truthy
      expect(day_four.horizontal_direction?(1, 1, 1, 0)).to be_truthy
      expect(day_four.horizontal_direction?(0, 0, 1, 0)).to be_falsey
      expect(day_four.horizontal_direction?(0, 0, 1, 1)).to be_falsey
    end

    it 'checks if the direction is vertical' do
      expect(day_four.vertical_direction?(0, 0, 1, 0)).to be_truthy
      expect(day_four.vertical_direction?(1, 1, 0, 1)).to be_truthy
      expect(day_four.vertical_direction?(0, 0, 0, 1)).to be_falsey
      expect(day_four.vertical_direction?(0, 0, 1, 1)).to be_falsey
    end
